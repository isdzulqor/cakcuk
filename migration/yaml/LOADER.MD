...
This SSH configurations is used to allow the feature of [**HTTPs over SSH**](https://github.com/isdzulqor/cakcuk/pull/22).
...
> 
> You can also inject loader configuration via environment variable `LOADER`. This is useful when you want to inject the configuration via CI/CD pipeline and store the `LOADER` in the secret. The value of the `LOADER` environment variable should be a base64-encoded string of the loader YAML configuration.
